...
def eval_single(annotation_file, result_file):
    ...
    for result in results:
        ...
        pred = result['text']
        # pred = truncate_before_newline(pred)
        pred = pred[1:] if len(pred) > 0 and pred[0] == ' ' else pred
        pred = pred[:-1] if len(pred) > 0 and pred[-1] == '.' else pred
        ground_truth = ground_truth[:-1] if len(ground_truth) > 0 and ground_truth[-1] == '.' else ground_truth
